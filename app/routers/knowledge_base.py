...
@router.post("/")
def create_knowledge_base(name: str = Form(...), description: str = Form(None)):
    conn = get_connection()
    cursor = conn.cursor()
...
@router.put("/{kb_id}")
def update_knowledge_base(kb_id: str, name: str = Form(None), description: str = Form(None)):
    conn = get_connection()
    cursor = conn.cursor()
    updates = []
    params = []
...
def list_documents(kb_id: str):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM documents WHERE knowledge_base_id = ? ORDER BY created_at DESC", (kb_id,))
    rows = cursor.fetchall()
    conn.close()
    return [dict(row) for row in rows]
...
async def upload_document(kb_id: str, file: UploadFile = File(...)):
    ...
    cursor.execute("""
        INSERT INTO documents (id, knowledge_base_id, filename, file_path, file_type, content, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    """, (doc_id, kb_id, file.filename, file_path, file_type, content, now, now))
    conn.commit()
    conn.close()
...
def get_document(kb_id: str, doc_id: str):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM documents WHERE id = ? AND knowledge_base_id = ?", (doc_id, kb_id))
    row = cursor.fetchone()
    conn.close()
    if not row:
...
def delete_document(kb_id: str, doc_id: str):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("DELETE FROM documents WHERE id = ? AND knowledge_base_id = ?", (doc_id, kb_id))
    conn.commit()
    conn.close()
    return {"message": "删除成功"}
